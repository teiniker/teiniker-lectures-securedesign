...
After starting the server, we can use a Browser to access the application's page:
    ...
    URL: https://lab.se.org:8443/Servlet-SSL-SimpleLogin/

Note that it's also possible to access the page via regular HTTP:

	URL: http://lab.se.org:8080/Servlet-SSL-SimpleLogin/
...
$ curl -k -i --verbose --proxy localhost:8010 -X GET https://localhost:8443/Servlet-SSL-SimpleLogin/
...
How to configure a HTTPS connection?
-------------------------------------------------------------------------------
*) Create a Keystore

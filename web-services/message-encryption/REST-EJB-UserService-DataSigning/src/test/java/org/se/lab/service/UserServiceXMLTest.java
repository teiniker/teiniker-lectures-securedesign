...
{
	private static final String WEB_APP_NAME = "/REST-EJB-UserService-DataSigning/v1";
	...
	public void testInsert() throws IOException, JAXBException
	{
		...
		String requestContent = 
				...
				+   "<sign>" + MessageSignature.generateHMacString("0,maggie,AZ2wv9X4WVHLRuRFLpZChYwAQVU=") + "</sign>"
				+ "</user>";
    ...
    @Test
    public void testInsert_InvalidSignature() throws IOException, JAXBException
    {
        // Request
        URL url = new URL("http://" + HOST + ":" + PORT + WEB_APP_NAME + "/users");
        String requestContent = 
                ...
                +   "<username>maggie</username>"
                +   "<password>AZ2wv9X4WVHLRuRFLpZChYwAQVU=</password>"
                +   "<sign>" + MessageSignature.generateHMacString("0,maggiXX,AZ2wv9X4WVHLRuRFLpZChYwAQVU=") + "</sign>"
                + "</user>";
        ...
        // Response
        int httpResponseCode = connection.getResponseCode();
        Assert.assertEquals(406, httpResponseCode); // 406 Not Acceptable
        Assert.assertEquals(0, connection.getContentLengthLong());
    }
	...
	public void testFindById() throws IOException, JAXBException
	{
		...
		final String EXPECTED = 
				...
				+   "<sign>" + MessageSignature.generateHMacString("3,bart,Ls4jKY8G2ftFdy/bHTgIaRjID0Q=") + "</sign>"
				+ "</user>\n";		
	...
	public void testFindAll() throws IOException, JAXBException
	{
		...
		final String EXPECTED = 
				...
				+       "<sign>" + MessageSignature.generateHMacString("1,homer,ijD8qepbRnIsva0kx0cKRCcYysg=") + "</sign>"
				+ 	"</user>"
				+ 	"<user id=\"2\">"
				+ 		"<username>marge</username>"
				+ 		"<password>xCSuPDv2U6I5jEO1wqvEQ/jPYhY=</password>"
				+       "<sign>" + MessageSignature.generateHMacString("2,marge,xCSuPDv2U6I5jEO1wqvEQ/jPYhY=") + "</sign>"
				+ 	"</user>"
				+ 	"<user id=\"3\">"
				+ 		"<username>bart</username>"
				+ 		"<password>Ls4jKY8G2ftFdy/bHTgIaRjID0Q=</password>"
				...
				+       "<sign>" + MessageSignature.generateHMacString("4,lisa,xO0U4gIN1F7bV7X7ovQN2TlSUF4=") + "</sign>"
				+ 	"</user>"
				+ "</collection>\n";
		System.out.println("Response-Content:\n" + content);
		Assert.assertEquals(EXPECTED, content);
